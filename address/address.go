...
import (
	...
	"github.com/venanetwork/quadrant/utils"
...
// vena uses a non-standard base32 character set.
const EncodeVena = "13456789abcdefghijkmnopqrstuwxyz"

var VenaEncoding = base32.NewEncoding(EncodeVena)
...
func AddressToPub(account types.Account) (public_key []byte, err error) {
	...
	if address[:4] == "xrb_" {
		address = address[4:]
	} else if address[:5] == "vena_" {
		address = address[5:]
	} else {
		return nil, errors.New("Invalid address format")
	}
	// A valid vena address is 64 bytes long
	// First 5 are simply a hard-coded string vena_ for ease of use
	// The following 52 characters form the address, and the final
	// 8 are a checksum.
	// They are base 32 encoded with a custom encoding.
	if len(address) == 60 {
		// The vena address string is 260bits which doesn't fall on a
		// byte boundary. pad with zeros to 280bits.
		// (zeros are encoded as 1 in vena's 32bit alphabet)
		key_b32vena := "1111" + address[0:52]
		input_checksum := address[52:]

		key_bytes, err := VenaEncoding.DecodeString(key_b32vena)
		if err != nil {
			return nil, err
		}
		...
		// vena checksum is calculated by hashing the key and reversing the bytes
		valid := VenaEncoding.EncodeToString(GetAddressChecksum(key_bytes)) == input_checksum
		if valid {
...
func PubKeyToAddress(pub ed25519.PublicKey) types.Account {
	...
	address := VenaEncoding.EncodeToString(padded)[4:]
	checksum := VenaEncoding.EncodeToString(GetAddressChecksum(pub))

	return types.Account("vena_" + address + checksum)
}
